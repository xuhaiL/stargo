...
func DistributeAuditDir(pluginName string) {
	...
	for i := 0; i < len(module.GYamlConf.FeServers); i++ {
		...
		// upload audit dir
		pluginDir := fmt.Sprintf("%s/%s-%s", module.GDownloadPath, pluginName, strings.Replace(module.GSRVersion, "v", "", -1))
		pluginTargetDir := fmt.Sprintf("%s/%s/%s", module.GYamlConf.FeServers[i].DeployDir, "localPlugins", pluginName)
		utl.UploadDir(sshUser, rsaKey, sshHost, sshPort, pluginDir, pluginTargetDir)
		infoMess = fmt.Sprintf("Upload dir pluginDir = [%s] to pluginTargetDir = [%s] on FeHost = [%s]", pluginDir, pluginTargetDir, sshHost)
		utl.Log("INFO", infoMess)
	}
