...
func main() {
	...
	case "plugin":
		...
		case "audit":
			clusterName = os.Args[3]
			utl.Log("OUTPUT", "INSTALL StarRocks Audit Plugins >>>>>>>>>>> ")
			plugins.DeployAudit(clusterName)
		case "un-audit":
			clusterName = os.Args[3]
			utl.Log("OUTPUT", "UNINSTALL StarRocks Audit Plugins >>>>>>>>>>> ")
			plugins.UninstallAudit(clusterName)
		}
